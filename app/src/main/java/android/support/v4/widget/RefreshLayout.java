...
//    private static final float REFRESH_DRAG_RATE = 1.2f;
    ...
    private static final int DEFAULT_CIRCLE_TARGET = 40;
    ...
    WechatMomentProgressDrawable mProgress;
...
//                mProgress.setAlpha(MAX_ALPHA);
    ...
    public RefreshLayout(Context context, AttributeSet attrs) {
        ...
        mOriginalOffsetTop = mCurrentTargetOffsetTop = -mCircleDiameter + 88;
        moveToStart(1.0f);
    ...
    private void createProgressView() {
        ...
        mProgress = new WechatMomentProgressDrawable(getContext(), this);
        mProgress.setBitmap(BitmapFactory.decodeResource(getResources(), R.mipmap.ic_moment));
        mCircleView.setImageDrawable(mProgress);
        mCircleView.setLayoutParams(new LayoutParams(mCircleDiameter, mCircleDiameter));
        mCircleView.setVisibility(View.GONE);
        addView(mCircleView);
    }
...
//                (width / 2 + circleWidth / 2), mCurrentTargetOffsetTop + circleHeight);
        mCircleView.layout(childLeft + CIRCLE_LEFT_OFFSET, mCurrentTargetOffsetTop,
                childLeft+CIRCLE_LEFT_OFFSET+circleWidth, mCurrentTargetOffsetTop + circleHeight);
    }
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        if (dy > 0 && mTotalUnconsumed > 0) {
            ...
            moveSpinner((int)(mTotalUnconsumed*0.6f));
            mTarget.setTranslationY((int)(mTotalUnconsumed*0.6f));
        }
    ...
    public void onStopNestedScroll(View target) {
        ...
        if (mTotalUnconsumed > 0) {
            ...
            mTotalUnconsumed = 0;
            mTarget.animate().translationY(0).setDuration(200).setInterpolator(new AccelerateDecelerateInterpolator()).start();
        }
...
//        mProgress.setArrowEnabled(true);
        ...
        dragPercent = Math.abs(originalDragPercent);
        adjustedPercent = (float) Math.max(dragPercent - .4, 0) * 5 / 3;
        float rotation = (-0.25f + .4f * adjustedPercent + tensionPercent * 2) / 6 * .5f;
        mProgress.setProgressRotation(rotation);
        setTargetOffsetTopAndBottom(targetY - mCurrentTargetOffsetTop);
    }

    private void finishSpinner(float overscrollTop) {
        if (overscrollTop > mTotalDragDistance+180) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged) {
                    ...
                    if (overscrollTop > 0) {
                        moveSpinner((int)(overscrollTop*0.6f));
                        mTarget.setTranslationY((int)(overscrollTop*0.6f));
                    } else {
                        return false;
                    }
                }
